...
export function createAdminSellerManagementService(prisma: PrismaClient) {
  ...
  async function deleteSeller(params: { sellerId: string; adminActorId: string }) {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.supportTicket.deleteMany({ where: { id: { in: supportTickets.map((ticket) => ticket.id) } } })
      ...
      await tx.discountRuleProduct.deleteMany({ where: { discountRuleId: { in: discountRuleIds } } })
    ...
    const storageKeys = [
      ...
      ...seller.documents.map((document) => document.fileKey),
    ]
