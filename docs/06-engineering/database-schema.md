...
### Product
`slug` drives `/urun/<slug>` routes. Only `published` products are returned in storefront queries. `price` and `compareAtPrice` use `Decimal` to avoid float rounding errors. Physical attributes (`weight`, `dimensionLength`, `dimensionWidth`, `dimensionHeight`) are stored for shipping calculations. Dimensions (cm) are optional and seller-entered from every upload path (single form, bulk Excel) — mapping **En → `dimensionWidth`, Boy → `dimensionLength`, Yükseklik → `dimensionHeight`**; when present they render on the product detail page next to the stock/fulfillment line.
...
Color and material are not scalar columns on `Product` — they are options in `ProductAttributeOption` (seeded from `db/seeds/attribute-options.ts`, unique on `[type, slug]`) linked to products via the `ProductAttributeValue` join. `ProductAttributeOption.sortOrder` is the **curated palette/dropdown order** (the seed writes the array index; `sortAttributeOptions` and the attribute-option APIs order by it). `ProductAttributeValue.sortOrder` is a **separate** column that orders a single product's colors: Renk 1 → 0, Renk 2 → 1 (material stays 0). The storefront joins a product's color labels by this order as `"Renk 1 - Renk 2"`. A product may hold two color values (or one "Mix" option for 3+ colors). New colors added to the seed appear in every category because no `CategoryAttributeOption` links are seeded (all dropdowns fall back to the full active list); reseed prod with `pnpm attributes:seed` (idempotent upsert).
