...
export async function readImageFileMetadata(file: File): Promise<ParsedImageMetadata> {
  const [{ width, height }, buffer] = await Promise.all([
    loadImageDimensions(file),
    file.slice(0, 64 * 1024).arrayBuffer(),
  ])
  const parsed = parseImageMetadata(new Uint8Array(buffer), file.type)
