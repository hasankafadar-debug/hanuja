...
export function AdminListControls({
  ...
  const activeFilters = useMemo(() => {
    ...
    return chips
  }, [barcodeValue, fromValue, invoiceOptions, invoiceValue, pageSize, sellerValue, statusOptions, statusValue, toValue])
  ...
  function resetFilters() {
    const next = new URLSearchParams(searchParams.toString())
    for (const key of ['q', 'barcode', 'status', 'invoice', 'seller', 'from', 'to', 'page']) {
      next.delete(key)
    }
    if (pageSize === 20) {
