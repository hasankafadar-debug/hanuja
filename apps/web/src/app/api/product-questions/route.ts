...
import {
  askProductQuestion,
  PRODUCT_QUESTION_ASK_RATE_LIMIT,
  PRODUCT_QUESTION_IP_RATE_LIMIT,
} from '@hanuja/api/routes/product-questions'

// POST /api/product-questions — yeni ürün sorusu (satış öncesi veya siparişe bağlı)
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return askProductQuestion(req, { id: session.user.id, role })
  } catch (err) {
